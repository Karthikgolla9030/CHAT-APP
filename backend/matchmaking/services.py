...
def calculate_match_score(user1, user2, prefs1):
    ...
    overlap = ui1 & ui2
    if overlap:
        score += min(0.45, len(overlap) * 0.15)
    ...
    if req_lang and req_lang.lower() == p2.language.lower():
        score += 0.20
    ...
    return max(0.0, min(1.0, score + random.uniform(-0.02, 0.02)))
...
def find_match_for_user(user, prefs):
    ...
    # Exclusion 2: Skipped Users in last 1 hour
    skipped_ids = set(SkippedUser.objects.filter(user=user, created_at__gte=timezone.now() - timedelta(hours=1)).values_list('skipped_user_id', flat=True))
